...
* *На сколько загружен CPU?*
В среднем опять около 25%:
![cpu](https://raw.githubusercontent.com/eremeykin/hh-jvm/report/img/stage_1/cpu.png)

* *Сколько в среднем потребляется памяти, заметен ли в программе memory leak?*
От 55 Мб до 130Мб. Пока не понятно чем вызван рост потребления памяти - утечкой или 
естественным увеличением количества пользоватлей в системе. В общем картина практически та же.
...
На этот раз `*DAOIml` создается по одному экземпляру, как по моему мнению и должно быть,
но я не уследил за созданием `H2DAOFactory`, можно сказать что это тоже объект без состояния 
и его не надо создавать больше одного, но их создается не много, оставим это как возможность
для дальнейших оптимизаций в случае необъодимости. 

* *Как часто происходит сборка мусора?*
За время эксперимента произошла 856 раз,  Средняя пауза 1 ms 971 μs, максимальная 103 ms 866 μs.
Изменения не несут практической значимости.

* *Cколько в среднем выполняется запуск сценария 1, как быстро увеличивается это время?*
Текущее время выросло с 0.03 с до 0.4с за 3920 выполнений сценария. Нет изменений.  
![pyplot](https://raw.githubusercontent.com/eremeykin/hh-jvm/report/img/stage_1/pyplot.png)


* *Какие операции из значимых (т.е. без учета работы системных функций, в т.ч. веб сервера) занимают больше всего процессорного времени?*
Все те же методы, улучшения нет
...
4) **Улучшение производительности не достигнуто**
...
* *На сколько загружен CPU?*
В среднем около 15%. Существенно лучше
![cpu](https://raw.githubusercontent.com/eremeykin/hh-jvm/report/img/stage_2/cpu.png)

* *Сколько в среднем потребляется памяти, заметен ли в программе memory leak?*
От 60 Мб до 600Мб. Вряд ли добавлена новая утечка (хотя исключить этого нельзя), 
скорее всего система стала более производительной и 
